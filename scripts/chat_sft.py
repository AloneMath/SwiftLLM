...
def main() -> None:
    ...
    try:
        ...
        if args.resume_ckpt:
            ...
            main_print(f"loaded base checkpoint: {args.resume_ckpt}")
        ...
        if dist_enabled:
            barrier()
        logger = RunLogger(sft_run_name, str(sft_ckpt_dir)) if is_main else None

        main_print(
            f"sft_run={sft_run_name} device={device} distributed={dist_enabled} "
            ...
            if step % args.save_every == 0:
                ...
                if is_main:
                    ...
                    save_checkpoint(
                        ...
                        model,
                        optimizer,
                        CheckpointState(step=step, best_val_loss=0.0),
                    )
        ...
        if is_main:
            ...
            torch.save({"model": final_model.state_dict(), "meta": {"stage": "sft", "step": args.sft_steps}}, final_ckpt)
